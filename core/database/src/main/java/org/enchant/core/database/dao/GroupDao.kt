...
class GroupDao(private val pool: DatabasePool) {
    suspend fun insert(group: GroupEntity) = pool.write { db ->
        db.execSQL("INSERT OR REPLACE INTO groups_table (group_id, name, description, avatar_media_id, my_role, member_count) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    suspend fun getAll(): List<GroupEntity> = pool.readWith { db ->
        db.rawQuery("SELECT * FROM groups_table ORDER BY name ASC", null).use { c ->
            ...
            while (c.moveToNext()) r.add(GroupEntity(c.getString(0), c.getString(1), c.getString(2) ?: "", c.getString(3) ?: "", c.getString(4), c.getInt(5)))
            r
        }
    }
